...
def get_initial_params(input_size, num_hidden, num_output):
    ...
    W1 = np.random.randn(input_size, num_hidden) * np.sqrt(1/input_size)
    b1 = np.zeros([num_hidden])
    W2 = np.random.randn(num_hidden, num_output) * np.sqrt(1/num_hidden)
...
def forward_prop(data, labels, params):
    ...
    Z2 = np.dot(a1, W2) + b2
    a2 = softmax(Z2)
    loss = (-1/m) * np.sum(np.sum(labels * np.log(a2)))

    #return a tuple containing activation, array of output, average loss
    return (a1, a2, loss)
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    #retrieve A1, A2 and loss from forward propgation
    a1, a2, loss = forward_prop_func(data, labels, params)

    #print(a1.shape, a2.shape, labels.shape)
    dZ2 = a2 - labels
    dW2 = (1/m) * np.dot(a1.T, dZ2)
    db2 = (1 / m) * np.sum(dZ2, axis=0, keepdims=True)

    #print(dZ2.shape, W2.shape, a1.shape)
    dZ1 = np.matmul(dZ2, W2.T) * (a1*(1-a1))
    dW1 = (1/m) * np.dot(data.T, dZ1)
    db1 = (1 / m) * np.sum(dZ1, axis=0, keepdims=True)

    #dictionary of dW1, db1, dW2, db2
    grads = {"dW1": dW1,
             "db1": db1,
             "dW2": dW2,
             "db2": db2}
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    # print(a1.shape, a2.shape, labels.shape)
    dZ2 = a2 - labels
    dW2 = (1 / m) * np.dot(a1.T, dZ2) + (reg * W2)
    db2 = (1 / m) * np.sum(dZ2, axis=0, keepdims=True)

    # print(dZ2.shape, W2.shape, a1.shape)
    dZ1 = np.matmul(dZ2, W2.T) * (a1 * (1 - a1))
    dW1 = (1 / m) * np.dot(data.T, dZ1) + (reg * W1)
    db1 = (1 / m) * np.sum(dZ1, axis=0, keepdims=True)
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for t in range(num_batch):
        mini_batch_X = train_data[t * batch_size: (t + 1) * batch_size, :]
        mini_batch_Y = train_labels[t * batch_size: (t + 1) * batch_size, :]


        #print(mini_batch_X.shape, mini_batch_Y.shape)

        a1, a2, loss = forward_prop_func(mini_batch_X, mini_batch_Y, params)

        #backward propogation
        grads = backward_prop_func(mini_batch_X, mini_batch_Y, params, forward_prop_func)
        # dW1 = grads['dW1']
        # db1 = grads["db1"]
        # dW2 = grads["dW2"]
        # db2 = grads["db2"]
